...
import (
	...
	response "service/order/Models/Response"
	"strconv"
	"time"

	cust "Service/Customer/Models/entity"
	prod "Service/Product/Models"
...
func GetProduct(ProductID int) (*prod.Product, error) {
	resp, err := http.Get(fmt.Sprintf("http://127.0.0.1:8003/product/%d", ProductID))
	if err != nil {
...
func GetOrdersByCustomerID(c *fiber.Ctx) error {
	...
	customer, err := GetLoggedInCustomer(cookie, customerID)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status":  "Failed",
			"message": err.Error(),
	...
	result := database.DB.Where("customer_id = ?", customerID).Find(&orders)
